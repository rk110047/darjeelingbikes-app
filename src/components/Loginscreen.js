...
import { View, StyleSheet, Image, TextInput, TouchableOpacity, Text, BackHandler ,AsyncStorage } from 'react-native';
...
export default class Loginscreen extends React.Component {
    ...
    //         }
    //         await fetch("http://3.131.162.213/auth/login/", {
    //             method: 'POST',
    //             headers: {
    ...
    async loginuser(){
        ...
        let res     =   await axios.post("http://3.142.212.172/auth/login/",
                    {
                        'mobile':this.state.username
                    })
        if(res.data.token){
